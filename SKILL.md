...
```text
Coordinator
    -> Worktree + Campaign / Variant / Run
    -> Assignment and Artifact Contracts
    -> ready
    -> Worker
    -> Completion Evidence
...
编排者的同步初始化范围在 `ready` 时结束。初始化包括创建工作树、按需创建专题/方案/执行目录与索引、合同、manifest、协议快照、启动约定、产物结构、上下文链接和合约校验。`ready` 仅表示初始化交接就绪，在编排报告或回复中记录，不是 Run、Campaign 或 Variant manifest 的新增状态；Run 的 `status` 和 `preflight.status` 均保持 `pending`，不能预填 Worker 的启动确认或执行结果。

在已有本地仓库、校验依赖可用且无需等待外部服务时，初始化目标耗时为 1–5 分钟，复杂仓库以 10 分钟以内为目标。这是操作目标，不是实测性能保证；超过 15 分钟必须向用户报告当前步骤、实际耗时及已知阻塞原因，未知时明确说明，不得仅凭耗时断言执行越界，也不得跳过检查或虚报 `ready`。

编排者不负责探测、准备或运行 CANN、驱动、固件、硬件、编译器或运行时环境，不负责运行 baseline、测试、构建、profiling 或性能测量。上述工作由被派发的 Worker、测试执行者或基准测试执行者在后续 Run 中完成。编排者只负责在合同中约定这些任务及其产物位置，并在后续验收时检查声明的证据和文件是否存在、路径是否正确、来源是否可追溯。参考资料和 baseline 目录可以由编排者创建约定，也可以按需由被授权的后续执行者创建；其内容不属于 worktree/Campaign 同步初始化。
...
```text
initialized -> planning -> provisioning -> ready -> dispatching -> monitoring -> validating
...
- `inputs` 列出执行者必须读取的文件，按角色纳入专题 README/索引、参考与验收文件、当前汇总、适用基线，以及方案计划和前序结论。编排者必须确认列出的全部输入已存在且可读；尚不存在的参考资料或基线应先分配准备 Run 生成，再派发依赖它们的任务，不得跳过缺失输入检查或在初始化中代为执行准备任务。执行期间可能变化的验收输入必须保存快照或版本，禁止无记录地改变已分配的标准。
...
新 Agent 不会继承编排者已加载的 Skill 或对话。下列第 1–3 项属于初始化准备，第 4–5 项属于后续派发与交接；仅请求创建工作树和专题时，在 `ready` 返回启动目录和合约/引导路径，不自动启动或等待 Worker。编排者必须：
...
使用下方校验器检查结构和路径证据，再由被授权的测试、基准测试或审计角色独立审查命令、测试覆盖、原始结果、环境和结论。Coordinator 不执行 CANN 或环境探测；它只核对这些角色提交的声明和证据文件。校验器不判断数值正确性、审计独立性、进程终止或业务验收。本协议提供交接门禁，不提供操作系统沙箱或命令白名单。
...
代码修改任务按以下阶段推进。第 1–7 步是同步初始化；第 7 步完成后返回 `ready`。第 8 步及以后属于已获授权的后续执行，不属于创建工作树和专题的完成条件：

1. 发现实际主仓库路径和现有项目布局，检查说明、活跃工作树及未提交状态。
2. 获取项目或专题协调范围的编排所有权。
3. 定义任务、角色、负责人、基准引用、分支、工作树和执行 ID。
4. 从明确 commit 创建分支和工作树。
5. 按需创建专题和方案的目录、README/索引与初始记录，创建持久执行目录、不可变 `assignment.json` 和 `pending` manifest。只填写已知目标、约定与引用，禁止预生成空目录骨架或执行结论。
6. 添加仓库本地忽略规则，创建 `.agent-artifacts/` 链接，并验证忽略规则和分配的执行路径。
7. 冻结此次执行的协议快照，填充 `worker-start.md`，通过合约结构校验与 `--check-files` 检查，并核对初始 manifest 的身份、路径和 pending 状态；返回 `ready`、实际工作树/专题/方案/执行路径、精确 commit、合约和启动引导路径及待执行事项。
8. 后续派发时，在合约给定的绝对目录启动执行者，要求先读合约、再读启动引导，由执行者在修改源码前完成启动前握手。初始化不等待此握手。
...
参考资料包及方案记录必须在实现前由指定执行者准备并经相应角色审查；这不是第 7 步返回 `ready` 的条件。缺少必需参考资料时，先为准备任务创建合约，将待生成资料列为输出，按需授予共享文档写入权；不得将尚不存在的文件列入该任务的必读 `inputs`。依赖这些资料的实现任务待资料就绪后再创建合约和派发。适用基线由指定测试或基准测试执行者建立，并在接受比较性结论前完成审计；创建工作树本身不要求运行 baseline。
...
- 初始化校验通过后返回 `ready`；不等待 Worker 握手，不执行 CANN 环境探测、构建、测试或 baseline。
- 对活跃执行者的执行内容只读。
...
仅为新任务初始化工作树和专题时，编排者读取仓库约束、相关索引、所有权和必要的前序引用即可；不要求遍历全部历史证据或重做环境与 baseline 验证。以下完整恢复流程用于实际接续工程任务，按角色与任务范围选择相关记录。
...
以下是执行交接和专题收尾的检查清单，不是初始化完成条件。仅创建工作树和专题时，按端到端生命周期第 1–7 步校验并报告 `ready`；无需生成终态 manifest、report、validation、测试或审计结果，也不得声称工程任务已完成。

执行交接或专题收尾时，所有适用检查通过前，禁止报告完成：
